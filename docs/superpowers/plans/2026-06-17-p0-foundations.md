...
> ✅ **EXECUTED & COMPLETE — 2026-06-19.** All 12 tasks implemented on `main`, CI green, full stack verified end-to-end. Commits `37bb84c..1bb80a5`. The two pre-execution decisions were taken (see [DECISION_LOG.md](../../DECISION_LOG.md) D31/D32): **RLS adopted now** (Task 7 gained a `hearth_app` role + `SET LOCAL ROLE` + policy, proven to block a forgotten `WHERE`) and **app containerization added** as **Task 12** (Dockerfiles + migrate-on-boot + Caddy → real `docker compose up`). Notable deviations forced by gaps the tests caught, all fixed: `drizzleAdapter` needs the schema passed explicitly; server tsconfig split (`tsconfig.build.json`) so `test/` helpers don't break typecheck; `apps/web/Dockerfile` builds the SPA + Caddy serves it; `matchMedia` jsdom stub; server vitest `singleFork` to stop a cross-file DB-truncate race; `vite-env.d.ts` + `declaration:false` for the app typechecks; CI pinned to pnpm 10 via `packageManager`. Local-only note: this dev box already runs Postgres on 5432, so a gitignored `docker-compose.override.yml` remaps the dev db to host **5433**.
