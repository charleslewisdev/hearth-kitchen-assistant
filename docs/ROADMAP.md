...
The monorepo is live and running: `docker compose up` brings up Postgres + Fastify/tRPC/Better-Auth API + an installable React PWA behind Caddy on one origin. A user can sign up, create a household, and add household-scoped recipes (tenant isolation enforced by both the repo layer and Postgres RLS). CI gates lint + typecheck + tests on green. A pre-P1 hardening pass (PR #1) automated RLS-coverage (a build-time guard fails if any `household_id` table lacks RLS), fixed local DB dev-ex (`pnpm test:local`, dotenv in `db:migrate`), and bumped CI actions off the deprecated Node 20 runtime — closing backlog #5 and #7. Remaining P0 deferrals are tracked in [the deferred/hardening backlog](superpowers/plans/2026-06-19-p0-deferred-and-hardening.md). The **P1 recipe-core design is approved** ([design doc](superpowers/specs/2026-06-21-p1-recipe-core-design.md)) and the **implementation plan is written** ([P1 plan](superpowers/plans/2026-06-21-p1-recipe-core.md), 5 phases A–E, TDD-ordered). **Next concrete step:** execute the P1 plan (likely in an isolated worktree), phase by phase with review checkpoints.
...
- **2026-06-21** — **Brainstormed & approved the P1 (Recipe core) design** → [design doc](superpowers/specs/2026-06-21-p1-recipe-core-design.md). Resolved five scope forks: thin-but-real `food` entity, linear-only scaling (`scaling_mode` stored for P4), natural-line ingredient entry backed by a reusable deterministic parser, **relational over JSON/raw-text** (locked as a standing preference: `recipe_step` rows + normalized `tag`/`recipe_tags`, no JSON arrays), and an additive-only forward-compat rule (P1 locks table identities; later phases only add columns/tables). Settled the 6-table schema, the isomorphic parser, FTS + three relational filters, and the explicit OUT-of-P1 boundary mapped to where each deferral lands. **Next: `writing-plans` → detailed P1 implementation plan, then execute.**
